...
const onGridClick = (e, worldBoard, canvasEl, centers, ctx) => {
  ...
  tileType.innerHTML = tile.type;
  let tileMayBuild = document.getElementById("tile-may-build");
  tileMayBuild.innerHTML = `may build: ${tile.attributes['mayBuild']}`;
  let tileMayPurchase = document.getElementById("tile-may-purchase");
  tileMayPurchase.innerHTML = `may purchase: ${tile.attributes['mayPurchase']}`;
  let tileEncounterChance = document.getElementById("tile-encounter-chance");
  tileEncounterChance.innerHTML = `encounter chance: ${tile.attributes['encounterChance']}`;
  let tileDifficultTerrain = document.getElementById("tile-difficult-terrain");
  tileDifficultTerrain.innerHTML = `difficult terrain: ${tile.attributes['difficultTerrain']}`;

  tile.addUnit(new __WEBPACK_IMPORTED_MODULE_1__unit__["a" /* default */]('archer'));
};
/* harmony export (immutable) */ __webpack_exports__["d"] = onGridClick;
...
const drawImage = (image, point, ctx) => {
  image.addEventListener("load", () => (
    ctx.drawImage(image, point[0] - 10, point[1] - 10, 20, 20)
  ));
};
...
const gridCoords = Object.keys(__WEBPACK_IMPORTED_MODULE_0_lodash___default.a.invert(GRID_COORDS)).map(coord => {
  coord = coord.split(',');
...
const renderWorldBoard = (worldBoard, ctx) => {
  ...
  for (let i = 0; i < gridCoords.length; i++) {
    ...
    point = __WEBPACK_IMPORTED_MODULE_0_lodash___default.a.invert(GRID_COORDS)[`${tile.x},${tile.y},${tile.z}`]
    ...
    if (tile.units.length > 0) {
      for (let j = 0; j < tile.units.length; j++) {
        ...
        if (j < 5) {
          drawImage(image, [point[0] - j * 3, point[1] - j * 3], ctx);
        } else {
          drawImage(image, [point[0] + 35 - j * 3, point[1] + 15 - j * 3], ctx);
        }
      }
    }
  }
};
/* harmony export (immutable) */ __webpack_exports__["e"] = renderWorldBoard;
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__tile__ = __webpack_require__(7);
...
class WorldBoard {
  ...
  buildGrid() {
    ...
    let gridCoords = Object.keys(__WEBPACK_IMPORTED_MODULE_2_lodash___default.a.invert(__WEBPACK_IMPORTED_MODULE_1__grid__["a" /* GRID_COORDS */])).map(coord => {
      coord = coord.split(',');
      return [parseInt(coord[0]), parseInt(coord[1]), parseInt(coord[2])];
    });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  canvasEl.addEventListener(
    "click",
  e => {
    Object(__WEBPACK_IMPORTED_MODULE_0__grid__["d" /* onGridClick */])(e, worldBoard, canvasEl, centers, ctx);
    Object(__WEBPACK_IMPORTED_MODULE_0__grid__["e" /* renderWorldBoard */])(worldBoard, ctx);
  });
...
const TILES = {
  ...
  orcLonghouse: [
    ...
    [
      new __WEBPACK_IMPORTED_MODULE_0__unit__["a" /* default */]('orc'),
      new __WEBPACK_IMPORTED_MODULE_0__unit__["a" /* default */]('orc')
    ],
    {
      orc: 2
  ...
  dwarfStronghold: [
    ...
    [
      new __WEBPACK_IMPORTED_MODULE_0__unit__["a" /* default */]('dwarf'),
      new __WEBPACK_IMPORTED_MODULE_0__unit__["a" /* default */]('dwarf')
    ],
    {
      dwarf: 2
  ...
  elfCitadel: [
    ...
    [
      new __WEBPACK_IMPORTED_MODULE_0__unit__["a" /* default */]('elf'),
      new __WEBPACK_IMPORTED_MODULE_0__unit__["a" /* default */]('elf')
    ],
    {
      elf: 2
